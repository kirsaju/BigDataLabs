...
# -------------------------------------------------------------------
print("Reading raw mock_data from PostgreSQL...")
raw = spark.read.jdbc(PG_URL, "mock_data", properties=PG_PROPS)
raw = raw.cache()
print(f"Total raw rows: {raw.count()}")

# -------------------------------------------------------------------
# 2. Build dimension tables (deduplicate by BUSINESS KEY)
# -------------------------------------------------------------------

# --- dim_customer: key = (first_name, last_name, email) ---
...
# --- dim_seller: key = (first_name, last_name, email) ---
...
# --- dim_product: key = (product_name, product_category) ---
...
# --- dim_store: key = (store_name, store_city) ---
...
# --- dim_supplier: key = (supplier_name, supplier_email) ---
...
# -------------------------------------------------------------------
# 3. Write dimension tables to PostgreSQL
# -------------------------------------------------------------------
...
print("All dimensions written.")

# -------------------------------------------------------------------
# 4. Build fact_sales by joining raw data with dimension IDs
# -------------------------------------------------------------------
print("Building fact_sales...")

# Re-read dimensions from PG — select only business key + ID (rename keys to avoid ambiguity)
...
# Parse sale_date in raw
raw_p = raw.withColumn("_sale_date_parsed", to_date(col("sale_date"), "M/d/yyyy"))

# Chain joins on business keys (renamed columns avoid ambiguity)
fact = raw_p \
...
# -------------------------------------------------------------------
# 5. Write fact_sales to PostgreSQL
# -------------------------------------------------------------------
...
print("fact_sales written to PostgreSQL.")

print("ETL to star schema complete!")
spark.stop()
